------------------------------------------------------------
-- dim_backprop.m2
-- Macaulay2 translation of the core ideas in dim_backprop.py
------------------------------------------------------------
...
-- If random(K) causes trouble in your installation, see note below.
randElt = K -> random(K);
...
-- Replacement for Sage's WeightedIntegerVectors
------------------------------------------------------------
...
------------------------------------------------------------
-- Feedforward
-- Hidden layers use entrywise power; last layer is linear
------------------------------------------------------------
...
backprop = (N, x, pullbackVector) -> (
    ...
    --------------------------------------------------------
    -- Forward pass:
    -- store only hidden preactivations / hidden activations
    -- final layer is linear
    --------------------------------------------------------

    activation := x;
    activations := new MutableList from {x};  -- input, then hidden activations
    zs := new MutableList from {};            -- hidden preactivations only
...
-- Input:
--   X = d0 x nsamples matrix
-- Output:
--   nsamples x dimPoly matrix
------------------------------------------------------------
...
------------------------------------------------------------
-- Choose enough independent rows to form an invertible
-- square submatrix
------------------------------------------------------------
...
------------------------------------------------------------
-- Recover coefficient matrix C from:
--
--    monomials * C = gradientsSamples
--
-- by choosing a square invertible row-submatrix
------------------------------------------------------------
...
recoverCoefficientMatrix = (monomials, gradientsSamples) -> (
    ...
    Msq := monomials_(idx, monoCols);
    Gsq := gradientsSamples_(idx, gradCols);
...
computeDimension = (networkWidths, networkExponent) -> (
    ...
    for p in primes do (
        ...
        for j from 0 to outDim - 1 do (
            ...
            gradientsRows := for i from 0 to nsamples - 1 list (
            ...
            gradientsSamples := matrix gradientsRows;
    ...
    nn0 := makeNetwork(networkWidths, networkExponent, first primes);
    sizes0 := nn0#"sizes";
    inDim0 := sizes0#0;
    outDim0 := sizes0#(#sizes0 - 1);

    ambientDim := binomial(nn0#"degree" + inDim0 - 1, inDim0 - 1) * outDim0;
    naiveBound := (sum apply(#networkWidths - 1, i -> (networkWidths#i - 1) * networkWidths#(i+1)))
...
restart
load"/Users/joserodriguez/Documents/GitHub/MFA_PNNs/dim_backprop-V2.m2"
